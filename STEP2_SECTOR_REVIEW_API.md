...
- **Sector Lead:** proposal `sector` must equal logged-in user's `sector` (wrong sector → `403`)
- **Super Admin:** any sector — full override
...
- **Approve / Reject** on `submitted` rows (any sector) → same PATCH endpoints as Sector Lead
- **View** → full proposal detail modal (`GET /api/proposals/:id`)
