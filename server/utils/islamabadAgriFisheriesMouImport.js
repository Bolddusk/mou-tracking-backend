...
const MOU_SUB_SECTOR = 'Fisheries & Aquaculture (including Processing)';
const PORTAL_SECTOR = 'Fisheries & Aquaculture (Including Processing)';

const ISLAMABAD_AGRI_FISHERIES_ROWS = [
  {
    sr: '1',
    chinese_company: 'Zhu Zhichao',
    pakistani_company: 'Legend International pvt ltd',
    cooperation_mode_raw: 'MoU',
    investment_value_usd: '15',
    description: 'For reprocessing of shrimp and crab and shell',
  },
];
...
function buildExternalReference(sr) {
  return `ISLAMABAD-AGRI-FISHERIES-${sr}`;
}
...
function getIslamabadAgriFisheriesRows() {
  return ISLAMABAD_AGRI_FISHERIES_ROWS.map((row) => ({
    ...row,
...
function buildProposalRecord(row, partyAId, sectorLeadId) {
  ...
  const conferenceInfo = buildConferenceInfo(ISLAMABAD_AGRI_2026, {
    description: `Historic ${row.cooperation_mode.toUpperCase()} imported from Islamabad Fisheries list (Sr ${row.sr}).`,
  });
...
module.exports = {
  ISLAMABAD_AGRI_FISHERIES_ROWS,
  getIslamabadAgriFisheriesRows,
  buildProposalRecord,
